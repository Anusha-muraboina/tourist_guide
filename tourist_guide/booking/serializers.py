...
from booking.models import Booking, CancelReason
...
# =====================================
# TOUR PRICING SERIALIZER  (GROUP BASED)
# =====================================
...
# =====================================
# BOOKING CREATE SERIALIZER  (GROUP BASED)
# =====================================
class BookingCreateSerializer(serializers.ModelSerializer):

    # Optional guide
    guide = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.filter(role="guide", is_active=True),
        required=False,
        allow_null=True
    )

    # Selected pricing (required)
    pricing = serializers.PrimaryKeyRelatedField(
        queryset=TourPricing.objects.filter(is_active=True),
        required=True
    )
    ...
    def validate(self, attrs):
        ...
        # ---------- Pricing belongs to this tour ----------
        if pricing and pricing.tour_id != tour.id:
            raise serializers.ValidationError({
                "pricing": "Selected pricing does not belong to this tour"
            })

        # ---------- Format time ----------
        if isinstance(tour_time, str):
            try:
                tour_time = datetime.strptime(tour_time, "%H:%M").time()
            except ValueError:
                try:
                    tour_time = datetime.strptime(tour_time, "%H:%M:%S").time()
                except ValueError:
                    raise serializers.ValidationError({
                        "tour_time": "Invalid time format. Use HH:MM or HH:MM:SS"
                    })
            attrs["tour_time"] = tour_time

        # ---------- Guide location check ----------
        if selected_guide:
        ...
        schedule = TourSchedule.objects.filter(
            tour=tour,
            is_active=True,
            start_time__hour=attrs["tour_time"].hour,
            start_time__minute=attrs["tour_time"].minute
        ).first()

        if not schedule:
            available_times = list(
                TourSchedule.objects.filter(
                    tour=tour, is_active=True
                ).values_list("start_time", flat=True)
            )
            raise serializers.ValidationError({
                "tour_time": f"This tour time is not available. Available times: {available_times}"
            })
        ...
        booked_slots = Booking.objects.filter(
            tour=tour,
            tour_date=tour_date,
            tour_time=attrs["tour_time"]
        ).exclude(status="cancelled").count()

        if booked_slots >= schedule.available_slots:
            raise serializers.ValidationError({
                "tour_time": "This slot is fully booked"
            })
    ...
    def create(self, validated_data):
        ...
        # ---------- Pop the fields we handle manually ----------
        pricing = validated_data.pop("pricing")
        group_members = validated_data.pop("group_members", 1)
        selected_guide = validated_data.pop("guide", None)
        coupon_code = validated_data.pop("coupon_code", None)

        # ---------- Price from selected group ----------
        group_price = Decimal(str(pricing.group_price)) if pricing else Decimal("0.00")

        sub_total = group_price
        ...
        if coupon_code:
            ...
            if not coupon.is_valid():
                raise serializers.ValidationError({
                    "coupon_code": "Coupon is expired or inactive"
                })
        ...
        booking = Booking.objects.create(
            ...
            status="pending",
            **validated_data          # now safe – pricing & guide already popped
        )
    ...
    def get_matching_guides(self, obj):
        ...
        return GuideSerializer(guides, many=True, context=self.context).data
    ...
    def get_available_slots(self, obj):
        ...
        return schedule.available_slots if schedule else 0
    ...
    def get_remaining_slots(self, obj):
        ...
        return max(schedule.available_slots - booked, 0)

    def get_group_price(self, obj):
        if obj.pricing:
            return obj.pricing.group_price
        return Decimal("0.00")
...
class BookingListSerializer(serializers.ModelSerializer):
    ...
    def get_guide_name(self, obj):
        return obj.guide.username if obj.guide else None

    def get_group_label(self, obj):
        if obj.pricing:
            return obj.pricing.get_group_type_display()
        return None
...
#         fields = [  "id",  "booking_id",  "tour",  "tour_details",  "guide",  "selected_guide",  "guest_name",  "guest_email",  "guest_phone",   
#           "tour_date",  "tour_time", "coupon_code", "special_requests", "payment_method", "payment_status", "adult_total", "child_total","infant_total",  "sub_total",
